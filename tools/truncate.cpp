...
int main(int argc, char **argv)
{
    if (argc < 6) {
        std::cout << "Usage:\n"
                  << "truncate in-file out-file out-bpc pack issigned\n";
    }

    const std::string infile_name(argv[1]);
    const std::string outfile_name(argv[2]);
    const int out_bpc = std::stoi(std::string(argv[3]));
    const bool pack = std::stoi(std::string(argv[4]));
    const bool issigned = std::stoi(std::string(argv[5]));
    ...
    for (int i = 0; i < n_blocks; i++) {
        ...
        for (int iw = 0; iw < read_size; iw++) {
            ...
            if (pack) {
                ...
                if (issigned && (truncated_word & (1 << (out_bpc - 1)))) {
                        out_block[out_i++] = 0xf000 | truncated_word;
                }
                else {
                    out_block[out_i++] = truncated_word;
                }
            }
            if (out_i == BUF_SIZE) {
